...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function shakeField(el) {
        ...
        el.style.borderColor = 'var(--color-error, #DC2626)';
        setTimeout(() => {
    ...
    async function goToStep(index) {
        ...
        if (index === 2) {
            const checkEl = steps[index].querySelector('.form-page__celebration-check');
            if (checkEl) checkEl.classList.add('is-animated');
    ...
    document.querySelectorAll('.form-page__option input[type="radio"], .form-page__scale-item input[type="radio"]').forEach((radio) => {
        radio.addEventListener('change', () => {
            const question = radio.closest('.form-page__question');
            if (question) question.classList.add('is-answered');
        });
    });
